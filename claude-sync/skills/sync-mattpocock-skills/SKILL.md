...
Two more skills — `implement-spec` and `retro` — were installed on explicit request from `skills/in-progress/`. The normal sync does **not** track them (it diffs only the shipped trees). If upstream later graduates either into `skills/engineering/` or `skills/productivity/`, the compare reports it as `renamed` from `in-progress/`; do **not** re-vendor it as a new skill or under a `matt-` prefix — the local directory already exists. Port only the content delta onto the existing copy, keeping its locally rewritten description.
