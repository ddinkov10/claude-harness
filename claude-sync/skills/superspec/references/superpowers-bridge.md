...
0. **Claude Code plugin (this harness)**: if a superpowers skill is invokable via the Skill
   tool (namespaced `superpowers:{skill-name}`), it is **available** — invoke it directly
   wherever it lives and skip the filesystem checks below. True whenever the obra/superpowers
   plugin is installed in Claude Code; takes precedence over paths 1–2.
1. **Project-local**: `$PROJECT_DIR/.agents/skills/{skill-name}/SKILL.md`
2. **User-global**: `~/.agents/skills/{skill-name}/SKILL.md`

A skill is considered **available** if it is invokable via the Skill tool, or if its
`SKILL.md` file exists at either path. Project-local skills take precedence over user-global.
