...
0. **Claude Code plugin (this harness)**: if a superpowers skill is available via the
   Skill tool (namespaced `superpowers:{skill-name}`), it is **available** — invoke it
   directly and skip the filesystem checks below. This is the case whenever the
   obra/superpowers plugin is installed in Claude Code, and takes precedence over paths 1–2.
