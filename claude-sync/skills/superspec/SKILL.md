...
the Skill tool (e.g. `superpowers:brainstorming`), regardless of filesystem location. Treat
all of them as **detected** and invoke them directly wherever they live; skip the
`~/.agents/skills/` filesystem probe. Fall back to the built-in protocols only if a Skill
