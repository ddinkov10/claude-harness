...
**Claude Code harness**: obra/superpowers is installed as a Claude Code plugin, so its
skills — `brainstorming`, `writing-plans`, `executing-plans`, `subagent-driven-development`,
`test-driven-development`, `requesting-code-review` — are already loaded and invokable via
the Skill tool (e.g. `superpowers:brainstorming`). Treat all of them as **detected** and
invoke them directly; skip the `~/.agents/skills/` filesystem probe, which does not apply
in this harness. Fall back to the built-in protocols only if a Skill invocation is genuinely
unavailable.
